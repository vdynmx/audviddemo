...
export class WebRTCAdaptor
{
	constructor(initialValues){
		...
		if (typeof navigator.mediaDevices == "undefined" && this.isPlayMode == false) {
			console.debug("Cannot open camera and mic because of unsecure context. Please Install SSL(https)");
	...
	getDevices(){
		navigator.mediaDevices.enumerateDevices().then(devices => {
			...
			if(checkAudio == false && this.localStream == null){
				console.debug("Audio input not found")
				console.debug("Retrying to get user media without audio")
	...
	leaveFromRoom(roomName) 
	{
		...
		console.debug ("leave request is sent for "+ roomName);
	...
	*/
	...
	iceCandidateReceived(event, streamId)
	{
		if (event.candidate) {
			...
			if (protocolSupported) {
				...
				if (this.debug) {
					console.debug("sending ice candiate for stream Id " + streamId );
					console.debug(JSON.stringify(event.candidate));
				}
				this.webSocketAdaptor.send(JSON.stringify(jsCmd));
			}
			else {
				console.debug("Candidate's protocol(full sdp: "+ event.candidate.candidate +") is not supported. Supported protocols: " + this.candidateTypes);
				if (event.candidate.candidate != "") { //
			...
			console.debug("No event.candidate in the iceCandidate event");
		}
	}
	...
	initDataChannel(streamId, dataChannel) 
	{
		dataChannel.onerror = (error) => {
			...
			console.debug("channel status: ", dataChannel.readyState);
	...
	initPeerConnection(streamId, dataChannelMode) 
	{
		if (this.remotePeerConnection[streamId] == null)
		{
			...
			console.debug("stream id in init peer connection: " + streamId + " close stream id: " + closedStreamId);
				...
				if (!this.isPlayMode) {
					if (this.remotePeerConnection[streamId].iceConnectionState == "connected") {
						...
						this.changeBandwidth(this.bandwidth, streamId).then(() => {
							console.debug("Bandwidth is changed to " + this.bandwidth);
						})
	...
	takeConfiguration(idOfStream, configuration, typeOfConfiguration)
	{
		...
		this.remotePeerConnection[streamId].setRemoteDescription(new RTCSessionDescription({
			...
			if (isTypeOffer) {
				...
				console.debug("try to create answer for stream id: " + streamId,type);
				console.debug(this.sdp_constraints,streamId,' ======== ',this.remotePeerConnection[streamId]);
				this.remotePeerConnection[streamId].createAnswer(this.sdp_constraints)
				.then(configuration =>
				{
					console.debug("created answer for stream id: " + streamId);
					this.gotDescription(configuration, streamId);
				})
				.catch((error) =>
				{
					console.debug("create answer error : " + error);
				});
			}

		}).catch((error) => {
			if (this.debug) {
				console.debug("set remote description is failed with error: " + error);
			}
	...
	{	
		...
		if (protocolSupported)
		{
			...
			.then(response => {
				if (this.debug) {
					console.debug("Candidate is added for stream " + streamId);
				}
			})
			.catch((error) => {
			...
			if (this.debug) {
				console.debug("Candidate's protocol("+candidate.protocol+") is not supported." +
	...
	getStats(streamId)
	{
		console.debug("peerstatsgetstats = " + this.remotePeerConnectionStats[streamId]);
