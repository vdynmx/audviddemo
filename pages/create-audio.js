...
import BlogForm from "../containers/Form/Audio"
...
const Extended = withTranslation('common', { i18n, wait: process.browser })(Blog);
...
Extended.getInitialProps = async function(context) {
    ...
    if(isServer){
      ...
      let id = ""
      if(context.query.audioId){
        id = "/"+context.query.audioId
      }
      const pageData = await axios.get(`/create-audio${id}?data=1`);
