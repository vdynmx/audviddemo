...
const LiveStreaming = (props) => (
  <Layout {...props} liveStreaming={true} >
    {
      props.pagenotfound ? 
        <PageNotFound {...props} />
        : props.user_login ?
...
const Extended = withTranslation('common', { i18n, wait: process.browser })(LiveStreaming);
...
Extended.getInitialProps = async function(context) {
    ...
    if(isServer){
      ...
      const pageData = await axios.get(`/live-streaming?data=1`);
