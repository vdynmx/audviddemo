...
const Extended = withTranslation('common', { i18n, wait: process.browser })(Video);
...
Extended.getInitialProps = async function(context) {
    ...
    if(isServer){
      ...
      let id = ""
      if(context.query.videoId){
        id = "/"+context.query.videoId
      }
      const pageData = await axios.get(`/create-video${id}?data=1`);
