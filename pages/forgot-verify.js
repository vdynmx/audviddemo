...
import ForgotForm from "../containers/Forgot/Verify"
...
const ForgotVerify = (props) => (
  <Layout {...props} loginButtonHide={true} >
    {
      props.pagenotfound ? 
        <PageNotFound {...props} />
: props.user_login ?
...
const Extended = withTranslation('common', { i18n, wait: process.browser })(ForgotVerify);
...
Extended.getInitialProps = async function(context) {
    ...
    if(isServer){
      ...
      const pageData = await axios.get("/reset/"+context.query.code+"?data=1");
      if(pageData.loggedInUserDetails){
        Router.push('/')
      }else{
